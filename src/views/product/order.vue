            ...
            <template v-if="advanced">
              <a-col :md="8" :sm="24">
                <a-form-item label="标题">
                  <a-input v-model="queryParam.label" placeholder=""/>
                </a-form-item>
              </a-col>
            </template>
...
import { getOrderList, updateOrderList, createOrderList, deleteOrderList } from '@/api/order'

import CreateForm from './modules/OrderCreateForm'
  ...
  methods: {
    ...
    handleOk () {
      ...
      form.validateFields((errors, values) => {
        if (!errors) {
          if (values.id > 0) {
            // 修改 e.g.
            updateOrderList(values).then(res => {
              this.visible = false
            ...
            // 新增
            createOrderList(values).then(res => {
              this.visible = false
              this.confirmLoading = false
              // 重置表单数据
              form.resetFields()
    ...
    handleSub (record) {
      ...
      this.$confirm({
        ...
        onOk () {
          deleteOrderList(record).then(res => {
            that.visible = false
